--- Fetch back-end for retrieving sources from GIT including Git submodules.
--module("luarocks.fetch.gitrec", package.seeall)
local gitrec = {}
...
function gitrec.get_sources(rockspec, extract, dest_dir, depth)
   assert(type(rockspec) == "table")
   assert(type(dest_dir) == "string" or not dest_dir)

   local git_cmd = rockspec.variables.GIT or "git"
   local name_version = rockspec.name .. "-" .. rockspec.version
   rockspec.source.url = rockspec.source.url:gsub("^gitrec", "git")
   ...
   local command = {fs.Q(git_cmd), "clone", depth or "--depth=1", "--recursive", rockspec.source.url, module}
   ...
   fs.delete(dir.path(store_dir, module, ".gitmodules"))
   fs.pop_dir()
   fs.pop_dir()
